...
calc_power_consumption <- function(input) {
  ...
  # Store gamma and epsilon bits by comparing row_sum to length of str_res
  # e.g. row_sum =
  gamma_bit <- sapply(row_sum, \(x) ifelse(x >= length(str_res)/2, 1, 0))
  epsil_bit <- sapply(row_sum, \(x) ifelse(x <  length(str_res)/2, 1, 0))

  # Convert bit to decimal for both gamma and epsilon
  gamma <- strtoi(paste0(gamma_bit, collapse = ""), base = 2L)
  epsilon <- strtoi(paste0(epsil_bit, collapse = ""), base = 2L)

  # caclulate power consumption
  gamma * epsilon
}

calc_power_consumption(input) # 3901196

# - Test ----
strtoi(10110, base = 2L)

# example input
test_input <- c("00100", "11110", "10110", "10111", "10101", "01111",
...
calc_power_consumption(test_input)
